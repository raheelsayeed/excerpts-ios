...
- (void)addImportedDataIntoLocaldatabase
{
    ...
    {
        Vignette * v = (Vignette *)[Vignette getOrCreateObjectWithIdentifier:d[kDefaultSyncAttributeName] attribute:kDefaultSyncAttributeName moc:_managedObjectContext propertiesDict:d];
        [v setType:@1];
...
- (void)startObserving
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(mergedChangesToMainContext:) name:@"mergedChangesToMainContext" object:nil];
}
...
- (void)stopObserving
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"mergedChangesToMainContext" object:nil];
...
-(void)mergedChangesToMainContext:(NSNotification *)notification
{
...
- (BOOL) isSyncableObject:(NSManagedObject *)mobject
{
    ...
    {
        DLog(@"NOT SYNCED: %@", [[mobject entity] name]);
        return NO;
    }
...
-(void)importVignettesFromLocalFiles:(NSArray *)fileList
{
    ...
    for(p=0; p< 535; p++)
    {
        ...
        Vignette *vig = [NSEntityDescription insertNewObjectForEntityForName:@"Vignette" inManagedObjectContext:managedObjectContext];
        vig.syncID = [Vignette syncID];
        vig.rawData = vignette;
        ...
        if(links.count > 0)
        {
            for(NSDictionary *ld in links)
            {
                ...
                link.service = ld.allKeys[0];
                NSDictionary *linkD = ld[link.service];
                if(ld[@"title"])
                    link.title = linkD[@"title"];
                link.data = linkD[@"link"];
