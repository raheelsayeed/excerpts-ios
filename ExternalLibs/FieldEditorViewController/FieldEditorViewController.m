...
- (id)initWithFieldSections:(NSArray *)sections tableStyle:(UITableViewStyle)style title:(NSString *)title
{
...
- (void)switchValueDidChange:(NamedSwitch *)switchControl
{
    ...
    if ((self.delegate) && ([delegate respondsToSelector:@selector(fieldEditor:switchChangedWithKey:withValue:)])) {
        [delegate fieldEditor:self switchChangedWithKey:switchControl.name withValue:@(switchControl.on)];
    }
...
-(void)segmentedControlValueDidChange:(NamedSegmentedController *)seg{
    self.hasChanges = YES;
    [self.values setObject:[NSNumber numberWithInteger:seg.selectedSegmentIndex] forKey:seg.name];
    
    if ((self.delegate) && ([delegate respondsToSelector:@selector(fieldEditor:segmentChangedWithKey:withValue:)])) {
        [delegate fieldEditor:self segmentChangedWithKey:seg.name withValue:[NSNumber numberWithInteger:seg.selectedSegmentIndex]];
    }
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ((type == FieldSpecifierTypeEmail) || (type == FieldSpecifierTypePassword) || (type == FieldSpecifierTypeURL) || (type == FieldSpecifierTypeText) || (type == FieldSpecifierTypeNumeric)) {
        ...
        switchControl.onTintColor = kColor_Orange;
    ...
    //::
        ...
        NamedSegmentedController *seg = [[NamedSegmentedController alloc] initWithItems:field.subsections];
        
        CGSize switchSize = seg.frame.size;
		CGSize contentSize = cell.contentView.bounds.size;
		CGRect switchFrame = CGRectMake(contentSize.width - switchSize.width - 8,
										8,
										switchSize.width,
										switchSize.height);
		seg.frame = switchFrame;

        seg.name = key;
        seg.tag  = SEGMENTTAG;
        seg.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
        [seg addTarget:self action:@selector(segmentedControlValueDidChange:) forControlEvents:UIControlEventValueChanged];
        [seg setSelectedSegmentIndex:[[self.values objectForKey:key] integerValue]];
  ...
  if (field.type == FieldSpecifierTypeButton) {
      if([field.defaultValue isKindOfClass:[UIColor class]]){
          cell.detailTextLabel.text = @"               ";
          cell.detailTextLabel.backgroundColor = field.defaultValue;
      }else if([field.defaultValue isKindOfClass:[NSString class]]){
          cell.detailTextLabel.text = field.defaultValue;
      }
      
      /*
      if([field.key  isEqualToString:@"f.color"]){
          cell.detailTextLabel.text = @"            ";
          cell.detailTextLabel.backgroundColor = field.defaultValue;
      }else if ([field.defaultValue isKindOfClass:[NSString class]]){
          cell.detailTextLabel.text = field.defaultValue;
...
{    
	...
	FieldSpecifier *field = [fields objectAtIndex:indexPath.row];
    if([field.key isEqualToString:@"f.delete"]){
        UIImageView *imageV = [[UIImageView  alloc] initWithImage:[UIImage imageNamed:@"Images/GUI/red-row"]];
...
- (void)openSubsection:(FieldSpecifier *)subsectionField withIdentifier:(NSString *)iden{
	...
	FieldEditorViewController *subController = [[FieldEditorViewController alloc] initWithFieldSections:sections tableStyle:UITableViewStyleGrouped title:@""]; 
	subController.preferredContentSize = self.preferredContentSize;
	subController.title = subsectionField.title;
    subController.editorIdentifier = iden;
...
@implementation NamedSegmentedController
...
+ (FieldSpecifier *)segmentedFieldWithKey:(NSString *)k title:(NSString *)segtitle defaultValue:(NSInteger)selectedSegment items:(NSArray *)baritems
{
    FieldSpecifier *field = [FieldSpecifier fieldWithType:FieldSpecifierTypeSegment key:k];
    field.title = segtitle;
    field.defaultValue = [NSNumber numberWithInteger:selectedSegment];
    field.subsections = baritems;
    return field;
}
