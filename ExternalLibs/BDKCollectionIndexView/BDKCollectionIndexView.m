...
- (void)layoutSubviews
{
    ...
    NSUInteger titlesCount = self.indexLabels.count;

    CGFloat otherDimension = floorf(maxLength / titlesCount);
    CGFloat labelHeight = fminf(otherDimension, self.theDimension);
    labelSize.height = labelHeight;
    
    CGFloat totalLabelsHeightMidY = (labelHeight * titlesCount ) / 2;
    CGFloat totalMidY             = maxLength / 2;
    CGFloat finalTopY             = totalMidY - totalLabelsHeightMidY;
   


    for (UILabel *label in self.indexLabels)
    {
        label.frame = (CGRect){{0, finalTopY}, labelSize};
        finalTopY += labelHeight;
    }
...
- (void)layoutSubviews {
    ...
    otherDimension = labelSize.height;
...
//        label.textColor = [UIColor colorWithRed:1.00 green:0.28 blue:0.50 alpha:1.00];
//        label.textColor = [UIColor colorWithRed:0.20 green:0.66 blue:0.86 alpha:1.00];
