...
- (void)viewDidLoad {
    ...
    if(_showHTMLActionButton)
        [self showHTMLActionButton];
}
...
- (void)viewWillAppear:(BOOL)animated {
  //  NSAssert(self.navigationController, @"SVWebViewController needs to be contained in a UINavigationController. If you are presenting SVWebViewController modally, use SVModalWebViewController instead.");
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        [self.navigationController setToolbarHidden:!_showToolbar animated:animated];
    }
}
...
#pragma mark - HTML BUtton

- (void)showHTMLActionButton
{
    UIBarButtonItem *htmlActionBtn = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(showHTMLActionActivity)];
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        self.navigationItem.rightBarButtonItems = @[fixedSpace, htmlActionBtn, flexibleSpace];
    }
    else
    {
        self.navigationItem.leftItemsSupplementBackButton = YES;
        self.navigationItem.leftBarButtonItems = @[htmlActionBtn];
    }
...
- (void)updateToolbarItems {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        if(_showCustomBackItemButton)
        {
        UIBarButtonItem * goBackInControllerView = [[UIBarButtonItem alloc] initWithTitle:@"↵ Back" style:UIBarButtonItemStylePlain target:self.navigationController action:@selector(goBack:)];
        self.toolbarItems = @[fixedSpace,
                              goBackInControllerView,
                              flexibleSpace,
                              self.backBarButtonItem,
                              flexibleSpace,
                              self.forwardBarButtonItem,
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if([[webView stringByEvaluatingJavaScriptFromString:@"document.title"] length] > 0)
...
- (void)showHTMLActionActivity
{
    NSString *html = [self.self.webView stringByEvaluatingJavaScriptFromString:@"document.documentElement.outerHTML"];
    
    UIActivityViewController *activityController = [[UIActivityViewController alloc] initWithActivityItems:@[html] applicationActivities:nil];
