...
static NSUInteger resultsTableTag = 1212;
...
- (void)setup {
	...
	_separator = [[UIView alloc] initWithFrame:CGRectMake(5, tokenFieldBottom, self.bounds.size.width-20, 0.5)];
	[_separator setBackgroundColor:[UIColor colorWithWhite:0.5 alpha:1]];
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        ...
        tableViewController.tableView.tag = resultsTableTag;
	...
	else
	{
        ...
        [_resultsTable setBackgroundColor:[UIColor darkGrayColor]];
        _resultsTable.tag = resultsTableTag;
...
{
    if(scrollView.tag == resultsTableTag)
    {
...
- (void)tokenFieldTextDidChange:(TITokenField *)field {
    ...
    {
        if(!_shouldSearchInBackground)[self setSearchResultsVisible:(_resultsArray.count > 0)];
    }
}
...
- (void)setSearchResultsVisible:(BOOL)visible {
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    {
		...
		if (visible)
        {
            [self presentpopoverAtTokenFieldCaretAnimated:YES];
        }
		else
        {
...
- (void)resultsForSearchString:(NSString *)searchString {
    ...
    {
        if(_shouldSearchInBackground) [self setSearchResultsVisible:NO];
    }
}
...
- (void) performSearch:(NSString *)searchString
{
  ...
  //added:
    ...
    if ([query rangeOfString:searchString options:NSCaseInsensitiveSearch].location != NSNotFound ||
        (_searchSubtitles && [querySubtitle rangeOfString:searchString options:NSCaseInsensitiveSearch].location != NSNotFound) ||
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
			...
			if (_tokens.count > 1 && untokSize.width > availableWidth){
				untokenized = [NSString stringWithFormat:@"%lu items", (unsigned long)titles.count]; //changed
			}
...
+ (UIColor *)blueTintColor {
    return [UIColor colorWithRed:0.81 green:0.93 blue:0.98 alpha:1.00];
...
- (void)drawRect:(CGRect)rect {
	...
	if (drawHighlighted){
		CGContextSetFillColor(context, (CGFloat[4]){red, green, blue, 0.4});
    ...
    //CGFloat highlightedComp[8] = {red, green, blue, 0.3, red, green, blue, 1};
    //CGFloat nonHighlightedComp[8] = {red, green, blue, 0.15, red, green, blue, 0.7};

    
	CGPathRef innerPath = CGPathCreateTokenPath(self.bounds.size, YES);
    CGFloat highlightedComp[8] = {red, green, blue, 0.4, red, green, blue, 0.4};
    ...
    //CGContextFillPath(context);
    CGContextSetFillColor(context, (drawHighlighted ? highlightedComp : nonHighlightedComp));
    CGContextFillPath(context);
